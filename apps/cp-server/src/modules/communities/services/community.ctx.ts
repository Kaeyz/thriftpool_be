...
export const validateCommunity = async (ctx: Ctx, communityKey?: string) => {
  if (!communityKey) throw new CtxError(StatusCodes.INVALID_INPUT, "Community Key is required in header");
  const { data: community } = await CommunityService.getCommunityByKey(ctx, communityKey);
  if (!community) throw new CtxError(StatusCodes.INVALID_INPUT, "Invalid Community key");
...
export const validateCommunityRole = async (ctx: Ctx, roles: string[]) => {
  ...
  if (!member) {
    const err = new CtxError(StatusCodes.UNAUTHORIZED, "User not a member of community");
